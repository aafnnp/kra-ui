...
describe('ToastManager', () => {
  ...
  test('getVisibleToasts: maxVisible=3 per placement, FIFO by createdAt', () => {
    ...
    const topVisible = getVisibleToasts([...toasts], 'top', { maxVisible: 3 });
    expect(topVisible.map((t) => t.id)).toEqual(['1', '2', '3']);

    const bottomVisible = getVisibleToasts([...toasts], 'bottom', { maxVisible: 3 });
  ...
  test('findDedupTarget: dedup within interval by status+message', () => {
    ...
    setNow(1000);
    const item1 = createToastItem(
      { message: 'same', status: 'success' },
      { dedupInterval: 1000 },
      deps,
    );
    const item2 = createToastItem(
      { message: 'diff', status: 'success' },
      { dedupInterval: 1000 },
      deps,
    );

    setNow(1500);
    const target = findDedupTarget(
      [item1, item2],
      { message: 'same', status: 'success' },
      { dedupInterval: 1000 },
      1500,
    ...
    const none = findDedupTarget(
      [item1],
      { message: 'same', status: 'error' },
      { dedupInterval: 1000 },
      1500,
    );

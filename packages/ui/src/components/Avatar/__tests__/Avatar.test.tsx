...
test('有 onPress 时点击触发回调（通过 accessibilityLabel="用户头像" 定位）', () => {
  ...
  const { getByLabelText } = renderWithProvider(
    <Avatar onPress={onPress} accessibilityLabel="用户头像" />,
  );
...
test('isDisabled 时不触发 onPress', () => {
  const onPress = jest.fn();
  const { getByLabelText } = renderWithProvider(
    <Avatar onPress={onPress} isDisabled accessibilityLabel="用户头像" />,
  );
...
test('传入 status 时渲染状态角标', () => {
  const { getByLabelText } = renderWithProvider(<Avatar status="online" />);
